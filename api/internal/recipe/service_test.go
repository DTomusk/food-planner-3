...
func TestGetRecipesByUserID_ActiveStatus(t *testing.T) {
	testutil.WithTx(t, func(tx *sql.Tx) {
		// Arrange
		txRunner := testutil.NewTestTxRunner(tx)
		ctx := context.Background()
		...
		activeRecipeID := uuid.New()
		err = seeds.InsertRecipe(ctx, tx, activeRecipeID.String(), testUser.ID.String(), "Active Recipe", 10, 20, 2, nil)
		require.NoError(t, err, "Failed to insert active recipe")

		// Create deleted recipe (should not be returned)
		deletedRecipeID := uuid.New()
		deletedOn := time.Now()
		err = seeds.InsertRecipe(ctx, tx, deletedRecipeID.String(), testUser.ID.String(), "Deleted Recipe", 10, 20, 2, &deletedOn)
		require.NoError(t, err, "Failed to insert deleted recipe")

		// Act
		recipes, err := s.GetRecipesByUserID(ctx, testUser.ID.String(), StatusActive, nil)

		// Assert
		require.NoError(t, err, "Expected no error when getting active recipes")
		require.Len(t, recipes, 1, "Expected to find one active recipe")
		require.Equal(t, activeRecipeID, recipes[0].ID, "Expected to find the active recipe")
		require.Equal(t, "Active Recipe", recipes[0].Name, "Expected correct recipe name")
	})
}

func TestGetRecipesByUserID_DeletedStatusWithoutViewerID(t *testing.T) {
	testutil.WithTx(t, func(tx *sql.Tx) {
		// Arrange
		txRunner := testutil.NewTestTxRunner(tx)
		ctx := context.Background()
		...
		// Act
		recipes, err := s.GetRecipesByUserID(ctx, testUser.ID.String(), StatusDeleted, nil)

		// Assert
		require.ErrorIs(t, err, ErrUnauthorized, "Expected ErrUnauthorized when viewerID is nil")
		require.Nil(t, recipes, "Expected nil recipes when unauthorized")
	})
}

func TestGetRecipesByUserID_DeletedStatusWithDifferentViewerID(t *testing.T) {
	testutil.WithTx(t, func(tx *sql.Tx) {
		// Arrange
		txRunner := testutil.NewTestTxRunner(tx)
		ctx := context.Background()
		...
		otherViewerID := otherUser.ID.String()

		// Act
		recipes, err := s.GetRecipesByUserID(ctx, testUser.ID.String(), StatusDeleted, &otherViewerID)

		// Assert
		require.ErrorIs(t, err, ErrUnauthorized, "Expected ErrUnauthorized when viewerID does not match userID")
		require.Nil(t, recipes, "Expected nil recipes when unauthorized")
	})
}

func TestGetRecipesByUserID_DeletedStatusWithMatchingViewerID(t *testing.T) {
	testutil.WithTx(t, func(tx *sql.Tx) {
		// Arrange
		txRunner := testutil.NewTestTxRunner(tx)
		ctx := context.Background()
		...
		// Create active recipe (should not be returned)
		activeRecipeID := uuid.New()
		err = seeds.InsertRecipe(ctx, tx, activeRecipeID.String(), testUser.ID.String(), "Active Recipe", 10, 20, 2, nil)
		require.NoError(t, err, "Failed to insert active recipe")
		...
		viewerID := testUser.ID.String()

		// Act
		recipes, err := s.GetRecipesByUserID(ctx, testUser.ID.String(), StatusDeleted, &viewerID)

		// Assert
		require.NoError(t, err, "Expected no error when getting deleted recipes with matching viewerID")
		require.Len(t, recipes, 1, "Expected to find one deleted recipe")
		require.Equal(t, deletedRecipeID, recipes[0].ID, "Expected to find the deleted recipe")
		require.Equal(t, "Deleted Recipe", recipes[0].Name, "Expected correct recipe name")
	})
}

func TestGetRecipesByUserID_InvalidStatus(t *testing.T) {
	testutil.WithTx(t, func(tx *sql.Tx) {
		// Arrange
		txRunner := testutil.NewTestTxRunner(tx)
		ctx := context.Background()
		...
		invalidStatus := RecipeStatus("INVALID")

		// Act
		recipes, err := s.GetRecipesByUserID(ctx, testUser.ID.String(), invalidStatus, nil)

		// Assert
		require.ErrorIs(t, err, ErrInvalidStatus, "Expected ErrInvalidStatus for invalid status")
		require.Nil(t, recipes, "Expected nil recipes for invalid status")
	})
}
